...
class Experiment:
    ...
    def evaluate(self, model, data):
        ...
        loggin.info("Number of data points: %d" % len(test_data_idxs))
    ...
    def train_and_eval(self):
        ...
        if model_name.lower() == "hyperplus":
            model = HypERPlus(d, self.ent_vec_dim,
            ...
            model = DistMult(d, self.ent_vec_dim,
                             self.rel_vec_dim, **self.kwargs)
        elif model_name.lower() == "conve":
            model = ConvE(d, self.ent_vec_dim, self.rel_vec_dim, **self.kwargs)
        elif model_name.lower() == "complex":
            model = ComplEx(d, self.ent_vec_dim,
                            self.rel_vec_dim, **self.kwargs)
        logging.info(f'Model parameters: {[value.numel() for value in model.parameters()]}')
